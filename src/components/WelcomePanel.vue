...
const fetchRecommendedQuestions = async (): Promise<QuestionCategory[]> => {
  ...
  return [
   {
      title: "园区政策咨询", icon: "Document", questions: [
        { text: "园区有哪些面向初创企业的优惠政策？", query: "查询初创企业优惠政策" },
        { text: "如何申请高新技术企业认定？", query: "高新技术企业认定申请流程及条件" },
        { text: "知识产权相关的扶持政策有哪些？" },
        { text: "关于人才引进和安居补贴的政策细则？" }
      ]
    },
    {
      title: "企业服务与办事指南", icon: "OfficeBuilding", questions: [
        { text: "新公司注册入驻园区需要哪些材料和流程？", query: "新公司入驻园区指南" },
        { text: "如何办理企业地址变更？" },
        { text: "在哪里可以查询和申报各类项目资金？" },
        { text: "园区提供哪些法律和财务咨询服务？" }
      ]
    },
    {
      title: "园区活动与资源对接", icon: "Calendar", questions: [
        { text: "近期有哪些技术交流或行业沙龙活动？", query: "查询近期园区活动" },
        { text: "如何报名参加创业大赛或路演？" },
        { text: "是否有投融资对接平台或服务？" },
        { text: "哪里可以找到合作供应商或技术伙伴？" }
      ]
    },
    {
      title: "基础设施与配套服务", icon: "Service", questions: [
        { text: "园区内有哪些餐厅和咖啡厅？", query: "园区餐饮介绍" },
        { text: "会议室、报告厅如何预订使用？" },
        { text: "园区班车路线和时刻表？" },
        { text: "健身房、图书馆等公共设施的开放时间和地点？" }
      ]
    },
  ];
};
...
.welcome-panel {
  ...
  position: relative; /* 作为绝对定位内容区的基准 */
  width: 100%;
  max-width: 800px;
...
.panel-header {
  ...
  padding: 10px 12px 5px; /* 调整内边距，底部留少许空间给图标 */
  /* height: 50px; */ /* 移除固定高度，让内容自适应 */
  ...
  z-index: 11;
  cursor: pointer; /* 表明整个头部可点击 */
  transition: background-color 0.2s; /* 添加悬停效果 */
}
.panel-header:hover {
    background-color: #f9fafb; /* 轻微背景变化 */
}
.panel-header:focus-visible { /* 键盘聚焦时的样式 */
...
.header-content {
  /* flex-grow: 1; */ /* 在 column 布局中不需要 */
  text-align: center; /* 标题居中 */
  /* padding-right: 30px; */ /* 移除右侧内边距 */
  margin-bottom: 2px; /* 标题和图标之间的间距 */
  width: 100%; /* 确保宽度撑满以便居中 */
}
...
.loading-indicator { display: flex; align-items: center; margin-bottom: 10px; }
.loading-indicator .dot { display: inline-block; width: 8px; height: 8px; margin: 0 3px; border-radius: 50%; background-color: #9ca3af; animation: loading-pulse 1.4s infinite ease-in-out both; }
.loading-indicator .dot:nth-child(1) { animation-delay: -0.32s; }
.loading-indicator .dot:nth-child(2) { animation-delay: -0.16s; }
@keyframes loading-pulse { 0%, 80%, 100% { transform: scale(0.7); opacity: 0.6; } 40% { transform: scale(1); opacity: 1; } }
...
.categories-container {
  ...
  margin-right: -5px; /* 抵消 padding */
}
.categories-container::-webkit-scrollbar { width: 5px; }
.categories-container::-webkit-scrollbar-thumb { background: #d1d5db; border-radius: 3px; }
...
.category-title { display: flex; align-items: center; flex: 1; min-width: 0; }
.category-title .el-icon { font-size: 16px; margin-right: 8px; color: var(--el-color-primary); flex-shrink: 0; }
.category-title h3 { font-size: 14px; font-weight: 600; color: #374151; margin: 0; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
.expand-icon { font-size: 12px; color: #9ca3af; transition: transform 0.3s ease-out; margin-left: 10px; flex-shrink: 0; }
...
.questions-list > div::-webkit-scrollbar { width: 4px; }
.questions-list > div::-webkit-scrollbar-thumb { background: #e0e0e0; border-radius: 2px; }
...
.question-item span { flex-grow: 1; margin-right: 8px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
.question-item:hover { background-color: #f3f4f6; color: #1f2937; }
