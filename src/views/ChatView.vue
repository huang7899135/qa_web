...
import type {
    ...
    RetrieverResource,
    Usage,
    // HistoryMessage as Message, // 确保 Message 类型定义是最新的
} from '@/api/index';
...
const handleSendMessage = async (payload: { query: string; files: File[] }) => {
  ...
  // ... (processChunk 和 handleError 逻辑保持不变，但移除 feedback 和 TTS 相关处理) ...
   ...
   // ... (启动流式请求逻辑保持不变) ...
    try {
        stopStreamHandler = (await sendChatMessageStream(request, processChunk, handleError)).abort;
...
.chat-messages-area {
  ...
  padding-bottom: 10px; /* Space between messages and input */
  width: 100%;
  max-width: 800px;
  margin-left: auto;
  margin-right: auto;
  padding-left: 5px; /* Adjust internal padding as needed */
  padding-right: 5px; /* Adjust internal padding as needed */
  box-sizing: border-box;
}

.chat-input-area {
  flex-shrink: 0; 
  padding-top: 10px; /* Add some space above the input box */
  padding-bottom: 10px; /* Space below input, above disclaimer */
  ...
  /* margin: 0 auto 10px auto; */ /* REMOVE this */
}
...
.chat-messages-area {
    ...
    scrollbar-color: #cccccc #f1f1f1; /* Firefox */
}
